		...
		if (::strcmp(host_feature->URI, LV2_URID_MAP_URI) == 0) {
			...
			if (m_urid_map) {
				m_urids.t101_ref_pitch = m_urid_map->map(
					m_urid_map->handle, PADTHV1_LV2_PREFIX "T101_REF_PITCH");
				m_urids.t102_ref_note = m_urid_map->map(
					m_urid_map->handle, PADTHV1_LV2_PREFIX "T102_REF_NOTE");
				m_urids.t103_scale_file = m_urid_map->map(
					m_urid_map->handle, PADTHV1_LV2_PREFIX "T103_SCALE_FILE");
				m_urids.t104_keymap_file = m_urid_map->map(
					m_urid_map->handle, PADTHV1_LV2_PREFIX "T103_KEYMAP_FILE");
...
bool padthv1_lv2::patch_put ( uint32_t ndelta )
{
