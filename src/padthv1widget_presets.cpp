...
#include "padthv1widget_presets.h"
...
//----------------------------------------------------------------------------
// padthv1widget_presets::ItemDelegate -- Custom (tree) list item delegate.

class padthv1widget_presets::ItemDelegate : public QItemDelegate
{
public:
	// ctor.
...
// ctor.
padthv1widget_presets::ItemDelegate::ItemDelegate (
...
// QItemDelegate interface...
QSize padthv1widget_presets::ItemDelegate::sizeHint (
...
QWidget *padthv1widget_presets::ItemDelegate::createEditor ( QWidget *pParent,
	const QStyleOptionViewItem& /*option*/, const QModelIndex& index ) const
{
	...
	case 0: // Text.
	{
		...
		padthv1widget_presets *pWidget
			= qobject_cast<padthv1widget_presets *>(parent());
		if (pWidget)
...
#ifdef CONFIG_DEBUG
	qDebug("padthv1widget_presets::ItemDelegate::createEditor(%p, %d, %d) = %p",
...
void padthv1widget_presets::ItemDelegate::setEditorData (
	QWidget *pEditor, const QModelIndex& index ) const
{
#ifdef CONFIG_DEBUG
	qDebug("padthv1widget_presets::ItemDelegate::setEditorData(%p, %d, %d)",
...
void padthv1widget_presets::ItemDelegate::setModelData ( QWidget *pEditor,
	QAbstractItemModel *pModel,	const QModelIndex& index ) const
{
#ifdef CONFIG_DEBUG
	qDebug("padthv1widget_presets::ItemDelegate::setModelData(%p, %d, %d)",
		pEditor, index.row(), index.column());
#endif
...
//----------------------------------------------------------------------------
// padthv1widget_presets -- UI wrapper form.

// ctor.
padthv1widget_presets::padthv1widget_presets ( QWidget *pParent )
	: QTreeWidget(pParent)
{
...
// dtor.
padthv1widget_presets::~padthv1widget_presets (void)
{
...
// utilities.
void padthv1widget_presets::loadPresets ( padthv1_presets *pPresets )
{
	QTreeWidget::clear();
	...
	while (bank_iter.hasNext()) {
		...
		padthv1_presets::Bank *pBank = pPresets->find_bank(sBank);
		if (pBank == nullptr)
			continue;
...
void padthv1widget_presets::savePresets ( padthv1_presets *pPresets )
{
	...
	for (int iItem = 0 ; iItem < iItemCount; ++iItem) {
		...
		//	const int iBankData = pBankItem->data(0, Qt::UserRole).toInt();
			padthv1_presets::Bank *pBank = pPresets->add_bank(sBank);
...
void padthv1widget_presets::loadPresetsCurrent ( padthv1_presets *pPresets )
{
	QString sBank;
	padthv1_presets::Bank *pBank
...
void padthv1widget_presets::savePresetsCurrent ( padthv1_presets *pPresets )
{
...
QString padthv1widget_presets::currentPreset (void) const
{
	QString sPreset;
...
// slots.
void padthv1widget_presets::addBankItem (void)
{
...
void padthv1widget_presets::addPresetItem (void)
{
	QTreeWidget::setFocus();
...
// factory methods.
QTreeWidgetItem *padthv1widget_presets::newBankItem (void)
{
	int iBank = 0;
	const int iItemCount = QTreeWidget::topLevelItemCount();
	for (int iItem = 0; iItem < iItemCount; ++iItem) {
...
QTreeWidgetItem *padthv1widget_presets::newPresetItem (void)
{
...
// file prompts.
//
QStringList padthv1widget_presets::openPresetFiles (void)
{
	QStringList files;
...
QString padthv1widget_presets::savePresetFile ( const QString& sPreset )
{
...
// config stuff.
//
void padthv1widget_presets::setPresetDir ( const QString& sPresetDir )
{
	m_sPresetDir = sPresetDir;
}

const QString& padthv1widget_presets::presetDir (void) const
{
	return m_sPresetDir;
}


void padthv1widget_presets::setPresetExt ( const QString& sPresetExt )
{
	m_sPresetExt = sPresetExt;
}

const QString& padthv1widget_presets::presetExt (void) const
{
	return m_sPresetExt;
}


void padthv1widget_presets::setDontUseNativeDialogs ( bool bDontUseNativeDialogs )
{
	m_bDontUseNativeDialogs = bDontUseNativeDialogs;
}

bool padthv1widget_presets::dontUseNativeDialogs (void) const
{
...
QValidator *padthv1widget_presets::presetsValidator (void) const
{
...
// dirty flag.
void padthv1widget_presets::setDirtyPresets ( bool bDirtyPresets )
{
	if (bDirtyPresets)
		++m_iDirtyPresets;
	else
...
bool padthv1widget_presets::isDirtyPresets (void) const
{
...
// private slots.
//
void padthv1widget_presets::itemChangedSlot ( QTreeWidgetItem *pItem, int )
{
...
void padthv1widget_presets::itemExpandedSlot ( QTreeWidgetItem *pItem )
{
	if (isBankItem(pItem))
		pItem->setIcon(0, QIcon(":/images/presetBankOpen.png"));
}

void padthv1widget_presets::itemCollapsedSlot ( QTreeWidgetItem *pItem )
{
	if (isBankItem(pItem))
...
void padthv1widget_presets::setPresetItem ( const QString& sPreset )
{
...
QTreeWidgetItem *padthv1widget_presets::presetItem ( const QString& sPreset ) const
{
	return nameItem(sPreset, PresetItem);
}

bool padthv1widget_presets::isPresetItem ( QTreeWidgetItem *pItem ) const
{
	return (pItem->type() == PresetItem);
}


void padthv1widget_presets::setBankItem ( const QString& sBank, int iPreset )
{
...
QTreeWidgetItem *padthv1widget_presets::bankItem ( const QString& sBank ) const
{
	return nameItem(sBank, BankItem);
}

bool padthv1widget_presets::isBankItem ( QTreeWidgetItem *pItem ) const
{
	return (pItem->type() == BankItem);
}


QTreeWidgetItem *padthv1widget_presets::nameItem (
...
QString padthv1widget_presets::nameRenum (
	const QString& sName, int iType ) const
{
...
QString padthv1widget_presets::bankRenum ( const QString& sBank ) const
{
	return nameRenum(sBank, BankItem);
}


QString padthv1widget_presets::presetRenum ( const QString& sPreset ) const
{
...
// Drag-n-drop stuff.
//
void padthv1widget_presets::mousePressEvent ( QMouseEvent *pMouseEvent )
{
...
void padthv1widget_presets::mouseMoveEvent ( QMouseEvent *pMouseEvent )
{
...
void padthv1widget_presets::mouseReleaseEvent ( QMouseEvent *pMouseEvent )
{
...
void padthv1widget_presets::dragEnterEvent ( QDragEnterEvent *pDragEnterEvent )
{
...
void padthv1widget_presets::dragMoveEvent ( QDragMoveEvent *pDragMoveEvent )
{
...
void padthv1widget_presets::dragLeaveEvent ( QDragLeaveEvent */*pDragLeaveEvent*/ )
{
	if (m_pRubberBand)
...
void padthv1widget_presets::dropEvent ( QDropEvent *pDropEvent )
{
	const QPoint& pos
	#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
...
bool padthv1widget_presets::canDropEvent ( QDropEvent *pDropEvent )
{
...
QTreeWidgetItem *padthv1widget_presets::dragDropItem ( const QPoint& pos )
{
...
void padthv1widget_presets::dropItem (
...
void padthv1widget_presets::moveRubberBand (
	QTreeWidgetItem *pDropItem, bool bOutdent )
{
	// We must be dragging something...
	QTreeWidgetItem *pDragItem = m_pDragItem;
...
void padthv1widget_presets::ensureVisibleItem ( QTreeWidgetItem *pItem )
{
...
void padthv1widget_presets::autoExpandTimeoutSlot (void)
{
	m_pAutoExpandTimer->stop();
...
// ctor.
padthv1widget_presets::ComboBox::ComboBox ( QWidget *pParent )
	: QComboBox(pParent),
	m_pPresetsView(new padthv1widget_presets(pParent))
{
...
padthv1widget_presets *padthv1widget_presets::ComboBox::presetsView (void) const
{
	return m_pPresetsView;
}


void padthv1widget_presets::ComboBox::setCurrentPreset ( const QString& sPreset )
{
...
QString padthv1widget_presets::ComboBox::currentPreset (void) const
{
	return QComboBox::currentText();
}


void padthv1widget_presets::ComboBox::resetCurrentPreset (void)
{
...
// end of padthv1widget_presets.cpp
