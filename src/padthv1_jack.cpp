...
void padthv1_jack::open ( const char *client_id )
{
	// init param ports
...
void padthv1_sigterm_handler ( int /*signo*/ )
{
	char c = 1;
	  ...
	  #endif
{
	...
	term.sa_handler = padthv1_sigterm_handler;
...
bool padthv1_jack_application::setup (void)
{
	...
	if (!nsm_url.isEmpty()) {
		...
		if (m_bGui)
			caps += "optional-gui:";
		m_pNsmClient->announce(PADTHV1_TITLE, caps.toLatin1().constData());
		if (m_pWidget)
...
void padthv1_jack_application::sigterm_handler (void)
{
	char c;
