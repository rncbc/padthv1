...
#include "padthv1widget_preset.h"
...
//-------------------------------------------------------------------------
// padthv1widget_preset - Custom preset-box widget.
//

// Constructor.
padthv1widget_preset::padthv1widget_preset ( QWidget *pParent )
	: QWidget(pParent)
{
	m_pNewButton    = new QToolButton();
	m_pOpenButton   = new QToolButton();
	m_pComboBox     = new padthv1widget_presets::ComboBox();
...
void padthv1widget_preset::clearPreset (void)
{
	++m_iInitPreset;
...
void padthv1widget_preset::setPreset ( const QString& sPreset )
{
	const bool bBlockSignals
		= m_pComboBox->blockSignals(true);
...
QString padthv1widget_preset::preset (void) const
{
...
bool padthv1widget_preset::queryPreset (void)
{
	if (m_iInitPreset == 0)
		return true;
...
void padthv1widget_preset::activatePreset ( const QString& sPreset )
{
...
void padthv1widget_preset::loadPreset ( const QString& sPreset )
{
	if (sPreset.isEmpty())
		return;
...
void padthv1widget_preset::newPreset (void)
{
	if (!queryPreset())
		return;
...
void padthv1widget_preset::openPreset (void)
{
	padthv1_config *pConfig = padthv1_config::getInstance();
	if (pConfig == nullptr)
		return;
...
void padthv1widget_preset::savePreset (void)
{
	savePreset(m_pComboBox->currentPreset());
}

void padthv1widget_preset::savePreset ( const QString& sPreset )
{
	padthv1_config *pConfig = padthv1_config::getInstance();
	if (pConfig == nullptr)
		return;
...
void padthv1widget_preset::deletePreset (void)
{
	const QString& sPreset
...
void padthv1widget_preset::resetPreset (void)
{
	const QString& sPreset
		= m_pComboBox->currentPreset();
...
void padthv1widget_preset::loadPresets (void)
{
	const bool bBlockSignals
		= m_pComboBox->blockSignals(true);
...
void padthv1widget_preset::savePresets (void)
{
	padthv1_config *pConfig = padthv1_config::getInstance();
	if (pConfig == nullptr)
		return;
...
// Preset control.
void padthv1widget_preset::initPreset (void)
{
	padthv1_config *pConfig = padthv1_config::getInstance();
...
// Dirty flag accessor.
void padthv1widget_preset::setDirtyPreset ( bool bDirtyPreset )
{
	if (bDirtyPreset)
		++m_iDirtyPreset;
	else
...
bool padthv1widget_preset::isDirtyPreset (void) const
{
	return (m_iDirtyPreset > 0);
}


void padthv1widget_preset::stabilizePreset (void)
{
...
QTreeWidgetItem *padthv1widget_preset::presetItem ( const QString& sPreset ) const
{
	return m_pPresetsView->presetItem(sPreset);
}


void padthv1widget_preset::setBankItem ( const QString& sBank, int iPreset )
{
	m_pPresetsView->setBankItem(sBank, iPreset);
}


QTreeWidgetItem *padthv1widget_preset::bankItem ( const QString& sBank ) const
{
	return m_pPresetsView->bankItem(sBank);
}


// end of padthv1widget_preset.cpp
