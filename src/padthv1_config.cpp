...
//
int padthv1_config::loadPresetsConf (
...
int padthv1_config::loadPresetsConfDir (
	padthv1_presets *pPresets, const QDir& dir, QStringList& confs )
{
	int nconfs = 0;
	...
	while (iter.hasNext()) {
		...
		if (!confs.contains(sFilename)) {
			padthv1_config::importPresets(sFilename, pPresets);
