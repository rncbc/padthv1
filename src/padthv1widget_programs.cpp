...
class padthv1widget_programs::ItemDelegate : public QStyledItemDelegate
{
public:
...
// painting
//
void padthv1widget_programs::ItemDelegate::paint ( QPainter *pPainter,
	const QStyleOptionViewItem& option, const QModelIndex& index ) const
{
	QStyleOptionViewItem opt = option;

	padthv1widget_programs *pWidget
		= qobject_cast<padthv1widget_programs *>(parent());
	if (pWidget) {
	...
	const QStyleOptionViewItem& /*option*/, const QModelIndex& index ) const
{
	...
	{
		if (index.parent().isValid()) {
			padthv1widget_presets::ComboBox *pComboBox
				= new padthv1widget_presets::ComboBox(pParent);
	...
	QWidget *pEditor, const QModelIndex& index ) const
{
	...
	case 1: // Text.
	{
		...
		if (index.parent().isValid()) {
			padthv1widget_presets::ComboBox *pComboBox
				= qobject_cast<padthv1widget_presets::ComboBox *> (pEditor);
			if (pComboBox) {
...
void padthv1widget_programs::addPresetItems ( padthv1_presets *pPresets )
{
	...
	while (preset_iter.hasNext()) {
		...
		padthv1_presets::Bank *pBank = pPresets->find_preset_bank(sPreset);
