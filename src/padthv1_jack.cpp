...
void padthv1_jack_application::show_error( const QString& msg )
{
#if defined(Q_OS_WINDOWS)
...
bool padthv1_jack_application::parse_args (void)
{
	...
	parser.addOption({{"n", "client-name"},
		QObject::tr("Set the JACK client name (default: %1)")
			.arg(PADTHV1_TITLE), "label"});
	parser.addHelpOption();
	parser.addVersionOption();
	parser.addPositionalArgument("preset-file",
		QObject::tr("Load preset file (.%1)").arg(PADTHV1_TITLE),
	...
	for (int i = 1; i < argc; ++i) {
		...
		if (sArg == "-h" || sArg == "--help") {
			...
			out << PADTHV1_TITLE " - " << QObject::tr(PADTHV1_SUBTITLE) + sEol;
			out << QObject::tr("Options:") + sEol;
			out << "  -g, --no-gui" + sEot +
				QObject::tr("Disable the graphical user interface (GUI)") + sEol;
			out << "  -n, --client-name=[label]" + sEot +
				QObject::tr("Set the JACK client name (default: %1)").arg(PADTHV1_TITLE) + sEol;
			out << "  -h, --help" + sEot +
...
void padthv1_jack_application::watchdog_start (void)
{
	if (g_pInstance)
